...
String outputLockState = "unlock"; // off is not locked
...
void setup() {
  ...
  // Set outputs to LOW
  digitalWrite(RELAY_PIN, LOW); // start with the lock locked
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            if (header.indexOf("GET /lock") >= 0) {
              Serial.println("lock is being locked");
              outputLockState = "lock";
              digitalWrite(RELAY_PIN, LOW); //lock
            } else if (header.indexOf("GET /unlock") >= 0) {
              Serial.println("lock is being unlocked");
              outputLockState = "unlock";
            ...
            client.println("<p>GPIO 16 - State is " + outputLockState + "ed</p>");
            // If the output26State is off, it displays the ON button       
            if (outputLockState=="unlock") {
